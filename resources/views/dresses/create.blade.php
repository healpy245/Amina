    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 text-right">
        <div class="bg-cream-100/80 dark:bg-neutral-900/80 overflow-hidden shadow-sm sm:rounded-2xl backdrop-blur-lg">
            <div class="p-6 text-neutral-700 dark:text-neutral-200">
                <form method="POST" action="{{ route('dresses.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4" class="block mt-1 w-full border-boutique-300 dark:border-neutral-700 dark:bg-neutral-900 text-boutique-900 dark:text-neutral-300 focus:border-boutique-500 dark:focus:border-boutique-600 focus:ring-boutique-500 dark:focus:ring-boutique-600 rounded-md shadow-sm">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="block mt-1 w-full border-boutique-300 dark:border-neutral-700 dark:bg-neutral-900 text-boutique-900 dark:text-neutral-300 focus:border-boutique-500 dark:focus:border-boutique-600 focus:ring-boutique-500 dark:focus:ring-boutique-600 rounded-md shadow-sm" required>
                            ...
                            <option value="under_measurement" {{ old('status') == 'under_measurement' ? 'selected' : '' }}>قيد القياس</option>
                            <option value="rented" {{ old('status') == 'rented' ? 'selected' : '' }}>مؤجر</option>
                            <option value="returned" {{ old('status') == 'returned' ? 'selected' : '' }}>مُعاد</option>
                            <option value="washing" {{ old('status') == 'washing' ? 'selected' : '' }}>في الغسيل</option>
                        </select>
