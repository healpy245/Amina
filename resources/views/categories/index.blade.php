                    ...
                    @forelse($categories as $category)
                        <tr onclick="window.location='{{ route('categories.show', $category) }}'" class="cursor-pointer hover:bg-boutique-100 dark:hover:bg-boutique-900 transition even:bg-cream-100/30 dark:even:bg-neutral-900/30" style="cursor:pointer;" data-id="{{ $category->id }}">
                            <td class="px-4 py-4 text-center">
                                <input type="checkbox" class="form-checkbox h-5 w-5 text-boutique-600 bg-boutique-800 border-boutique-600 rounded focus:ring-0 category-checkbox" onclick="event.stopPropagation();">
                            </td>
                            ...
                            <td class="px-6 py-4 flex gap-2 justify-end">
                                <a href="{{ route('categories.edit', $category) }}" onclick="event.stopPropagation();" class="bg-boutique-600 hover:bg-boutique-700 text-white px-4 py-2 rounded-xl font-bold shadow transition-all duration-300 flex items-center gap-1" title="تعديل">
                                ...
                                <form action="{{ route('categories.destroy', $category) }}" method="POST" onsubmit="event.stopPropagation(); return confirm('هل تريد حذف هذه الفئة؟');">
                                    @csrf
                                    @method('DELETE')
